...
- **`still_flagged` in the replay harness now agrees with `evaluate()`** about the markers `evaluate()` blanks. It did not, so a mark recorded against `FOOTNOTE`, `CAPTION` or `IMAGE` counted as still reported while the evaluator no longer emits it — and `removal_bucket`'s `footnote_marker` branch was consequently unreachable. Both now read one shared `_BLANKED_MARKER_TOKENS` set, so they cannot drift apart again, and the bucket is renamed `blanked_marker` for the two markers it also covers. The glossary is still deliberately left out of the guard: membership is a per-book editorial decision that keeps changing under the corpus, and folding it in would credit the code for removals a human made by adding a name to a list.
